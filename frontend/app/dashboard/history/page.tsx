...
import { mapMarketSummary, stroopsToUsdc } from "@/lib/blind-market";
import {
    History,
    CheckCircle2,
    XCircle,
...
import { getPrivyStellarWallet, loadMarketIds, loadMarketView, loadPosition, sellSharesWithPrivyWallet } from "@/lib/stellar";
...
function toHistoryDataSource(
    condition: { assetSymbol: string; oracleContract: string; comparator: ">=" | "<="; threshold: bigint },
    observedPrice?: bigint,
) {
    const source = TRUSTED_DATA_SOURCES.find((entry) => (
        entry.ticker === condition.assetSymbol || entry.oracleContract === condition.oracleContract
    ));
    ...
    return {
        id: source.id,
        currentValue: Number(observedPrice ?? condition.threshold),
        targetValue: Number(condition.threshold),
        operator: condition.comparator,
    };
}

function blindMarketToHistoryEntry(market: ReturnType<typeof mapMarketSummary>): MarketStrategy {
    const usedDataSources = market.conditions
        .map((condition, index) => toHistoryDataSource(condition, market.resolvedConditions[index]?.observedPrice))
        .filter((entry): entry is NonNullable<typeof entry> => Boolean(entry));
    const totalVolume = stroopsToUsdc(market.totalLockedCollateral);
    const winner = market.outcome === "YES" ? "yes" : market.outcome === "NO" ? "no" : null;

    return {
        id: market.marketId,
        name: market.question,
        description: market.oracleLogic,
        evaluationLogic: market.oracleLogic,
        mathematicalLogic: market.oracleLogic,
        usedDataSources,
        resolutionDeadline: market.settledAt ?? market.endTimestamp,
        yesToken: {
            volume: totalVolume,
            history: [],
            twap: winner === "yes" ? 1 : 0,
        ...
        timestamp: market.settledAt ?? market.endTimestamp,
        resolved: true,
        winner,
    };
}

function evaluateHistoryCondition(operator: string | undefined, currentValue: number, targetValue: number) {
...
export default function HistoryPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [state, fetchedAgents, graduated, marketIds] = await Promise.all([
                    api.getMarketState(),
                    api.getAgents(),
                    api.getGraduatedStrategies(),
                    loadMarketIds(),
                ]);
                const settledMarkets = await Promise.allSettled(
                    marketIds.map(async (marketId) => ({
                        marketId,
                        view: await loadMarketView(marketId),
                    })),
                );
                const resolvedBlindMarkets = settledMarkets
                    .flatMap((result) => result.status === "fulfilled" ? [mapMarketSummary(result.value)] : [])
                    .filter((market) => market.resolved)
                    .map(blindMarketToHistoryEntry);

                setMarketState(state);
                // Filter out duplicates by ID and sort by timestamp.
                const mergedHistory = [...resolvedBlindMarkets, ...(graduated || [])];
                const uniqueProposals = mergedHistory.reduce((acc, proposal) => {
    ...
    useEffect(() => {
        if (!stellarWallet?.address || historicalProposals.length === 0) return;
        ...
        const fetchBalances = async () => {
            ...
            try {
                ...
                const settledBalances = await Promise.allSettled(historicalProposals.map(async (p) => {
                    const position = await loadPosition(p.id, address);
                    return {
                        id: p.id,
                        balance: (Number(position.yes_shares) / 10_000_000).toFixed(4),
                    };
                }));
                settledBalances.forEach((result) => {
                    if (result.status === "fulfilled") {
                        balances[result.value.id] = result.value.balance;
                    }
                });
        ...
        fetchBalances();
    }, [stellarWallet?.address, historicalProposals]);

    const handleRedeem = async (id: string, amount: number) => {
        if (!stellarWallet || !authenticated) return;
        ...
        try {
            ...
            await sellSharesWithPrivyWallet(stellarWallet, signRawHash, {
                marketId: id,
                side: "YES",
                shareAmount: amountInStroops,
            });

            setRedeemedIds(prev => new Set(prev).add(id));

            const updatedPosition = await loadPosition(id, stellarWallet.address);
            setRealHoldings(prev => ({
                ...prev,
                [id]: (Number(updatedPosition.yes_shares) / 10_000_000).toFixed(4),
            }));

        } catch (error: any) {
            console.error("Redemption failed detailed:", error);
            setRedeemErrors(prev => ({ ...prev, [id]: error?.message || "Transaction failed. Please try again." }));
        } finally {
                        ...
                        {isInitialLoading ? (
                            ...
                            >
                                <div className="flex flex-col items-center gap-4">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-[0.4em] text-violet-300/60 ml-2">
                                        Loading History
                                    </span>
                                </div>
                            </motion.div>
                            ...
                            <div key="content" className="px-4 sm:px-6 md:px-8 lg:px-12 py-6 md:py-10 max-w-7xl mx-auto">
                                <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 md:mb-12">
                                    <div>
                                        ...
                                        <p className="text-white/40 font-medium text-sm md:text-lg uppercase tracking-widest">Private market outcomes and claim history</p>
                                    </div>
                                    <div className="flex gap-4">
                                        {historicalProposals.length > 0 && (
                                            <div className="p-3 md:p-4 bg-white/5 border border-white/10 rounded-xl md:rounded-2xl">
                                                <h3 className="text-[10px] font-bold text-white/20 uppercase tracking-widest mb-1">History Set</h3>
                                                <div className="text-xl md:text-2xl font-black text-violet-200 uppercase">
                                ...
                                {historicalProposals.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center py-32 px-4 text-center border-2 border-dashed border-white/5 rounded-3xl bg-white/[0.01]">
                                        ...
                                        <p className="text-base text-white/30 font-medium max-w-md mx-auto leading-relaxed">
                                            No resolved markets are available yet. Once markets settle, their outcomes and claim context will show up here.
                                        </p>
                                    </div>
                                ) : (
                                                ...
                                                >
                                                            ...
                                                            <div className="flex items-center gap-4 md:gap-6">
                                                                ...
                                                                <div>
                                                                    <div className="flex flex-wrap items-center gap-3 mb-1.5">
                                                                        ...
                                                                        <span className={`px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest border ${proposal.winner === 'yes'
                                                                            ? 'bg-violet-500/20 text-violet-100 border-violet-400/30'
                                                                            : 'bg-red-500/20 text-red-400 border-red-500/30'}`}>
                                                                            {proposal.winner === 'yes' ? 'YES' : 'NO'}
                                                                        </span>
                                                                    </div>
                                                                    <div className="flex items-center gap-4">
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-violet-300/60 hover:text-violet-200 transition-colors"
                                                                        >
                                                            ...
                                                            {expandedIds.has(proposal.id) && (
                                                                ...
                                                                >
                                                                    <div className="pt-8 space-y-8">
                                                                        <div>
                                                                                <div className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em] mb-3">Resolution Setup</div>
                                                                            <p className="text-white/60 text-[14px] leading-relaxed font-medium italic p-4 bg-white/[0.02] border-l-2 border-violet-400/20 rounded-r-xl font-mono">
                                                                        ...
                                                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                                                                            ...
                                                                            <div className="p-4 md:p-6 bg-white/[0.02] border border-white/5 rounded-2xl">
                                                                                <div className="text-[9px] md:text-[10px] font-black text-white/20 uppercase tracking-widest mb-2 md:mb-4">Final Outcome Mix</div>
                                                                                <div className="flex items-end gap-2 text-lg md:text-xl font-black text-white">
                                                                                    {(proposal.yesToken.twap * 100).toFixed(1)}% <span className="text-[9px] md:text-[10px] text-violet-300 mb-0.5 md:mb-1">YES</span>
                                                                                </div>
                                                                            </div>
                                                                            ...
                                                                            <div className="p-4 md:p-6 bg-white/[0.02] border border-white/5 rounded-2xl flex flex-col justify-between min-h-[80px] md:min-h-0">
                                                                                <div className="flex items-center justify-between mb-2">
                                                                                    <div className="text-[9px] md:text-[10px] font-black text-white/20 uppercase tracking-widest">Outcome Check</div>
                                                                                    <div className="flex items-center gap-1">
                                                                                        <div className="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-white/20" />
                                                                                        <span className="text-[9px] md:text-[10px] text-white/20 font-black uppercase tracking-tighter">Ready</span>
                                                                                    </div>
                                                                                </div>
                                                                                <button
                                                                                    ...
                                                                                    className={`w-full py-2 md:py-2.5 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 border ${isVerified
                                                                                        ? 'bg-violet-500/10 text-violet-100 border-violet-400/20'
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    {isVerifying ? 'Checking outcome...' : isVerified ? 'Outcome checked' : 'Check outcome'}
                                                                                </button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                    ...
                                                    {/* User Position & Redemption (Only if winner) */}
                                                    {
                                                        (() => {
                                                            ...
                                                            return (
                                                                <div className={`mx-4 md:mx-8 mb-6 p-4 md:p-5 rounded-2xl border transition-all ${isWinner ? 'bg-violet-500/[0.04] border-violet-400/12 shadow-sm' : 'bg-red-500/[0.01] border-red-500/5'}`}>
                                                                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                                                        <div className="flex items-center gap-4 w-full sm:w-auto">
                                                                            <div className={`hidden lg:flex w-10 h-10 rounded-xl shrink-0 items-center justify-center border transition-all duration-500 ${isWinner ? 'bg-violet-500/10 border-violet-400/20' : 'bg-red-500/10 border-red-500/20'}`}>
                                                                                <Wallet className={`w-5 h-5 ${isWinner ? 'text-violet-200' : 'text-red-400'}`} />
                                                                            </div>
                                                                            ...
                                                                            <div className="flex flex-1 items-center justify-between sm:justify-start sm:gap-8">
                                                                                ...
                                                                                <div className="flex flex-col">
                                                                                    <div className="flex items-baseline gap-1.5">
                                                                                        <span className={`text-xl md:text-2xl font-black tracking-tighter tabular-nums leading-none ${isWinner ? 'text-violet-100' : 'text-red-400/40 line-through'}`}>
                                                                                            ${isWinner ? amount.toLocaleString() : '0.00'}
                                                                                        </span>
                                                                                        <span className="text-[9px] md:text-[10px] font-black text-white/30 uppercase tracking-widest">USDC</span>
                                                                                    </div>
                                                                            ...
                                                                            <div className="flex flex-col items-end gap-2 w-full sm:w-auto">
                                                                                <button
                                                                                    ...
                                                                                    className={`w-full sm:w-auto px-6 h-10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 flex items-center justify-center gap-2 ${isRedeemed || redeemLoaders.has(proposal.id)
                                                                                        ? 'bg-violet-500/5 text-violet-300/40 border border-violet-400/10 cursor-not-allowed'
                                                                                        : 'bg-violet-500 text-white hover:bg-violet-400 hover:scale-105 active:scale-95 shadow-lg'
                                                                                        }`}
                                                                                >
                                                    ...
                                                    >
                                                        ...
                                                        {auditExpandedIds.has(proposal.id) ? 'Hide outcome details' : 'View outcome details'}
                                                    </button>
                                                            ...
                                                            >
                                                                <div className="p-4 md:p-8 space-y-6 md:space-y-8">
                                                                    ...
                                                                    {verifiedIds.has(proposal.id) ? (
                                                                        <div className="p-5 md:p-8 bg-black/60 rounded-2xl border border-violet-400/20 relative group/audit">
                                                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                                                                                <div className="text-[10px] font-black text-violet-100 uppercase tracking-[0.25em] flex items-center gap-2">
                                                                                    <CheckCircle2 className="w-3.5 h-3.5" />
                                                                                    Outcome check
                                                                                </div>
                                                                                <div className="inline-flex items-center gap-2 px-3 py-1 bg-violet-500/10 rounded-full border border-violet-400/20 w-fit">
                                                                                    <SearchCheck className="w-3 h-3 text-violet-100" />
                                                                                    <span className="text-[9px] font-black text-violet-100 uppercase tracking-tighter">Checked against live feeds</span>
                                                                                </div>
                                                                            </div>
                                                                            ...
                                                                            <div className="grid grid-cols-1 gap-3 mb-6">
                                                                                {sortedDataSources.map((ds, idx) => {
                                                                                    ...
                                                                                    const isConditionMet = evaluateHistoryCondition(ds.operator, effectiveValue, ds.targetValue);
                                                                                            ...
                                                                                            <div className="flex items-center gap-2 md:gap-4 shrink-0">
                                                                                                <div className="flex flex-col items-end">
                                                                                                    ...
                                                                                                    {verifiedVal !== undefined && (
                                                                                                        <span className="text-[8px] font-bold text-violet-300/60 uppercase tracking-tighter">Live checked</span>
                                                                                                    )}
                                                                                                </div>
                                                                                                <ArrowRight className="w-3 h-3 text-white/20" />
                                                                                                <span className={`text-[10px] md:text-xs font-mono font-bold px-2 py-0.5 rounded-md ${isConditionMet ? 'bg-violet-500/10 text-violet-100' : 'bg-red-500/10 text-red-400'}`}>
                                                                            ...
                                                                            <div className="pt-6 border-t border-white/5">
                                                                                {(() => {
                                                                                    const isAuditPassed = sortedDataSources.every(ds => {
                                                                                        const verifiedVal = verificationResults[proposal.id]?.[ds.id];
                                                                                        const effectiveValue = verifiedVal !== undefined ? verifiedVal : ds.currentValue;
                                                                                        return evaluateHistoryCondition(ds.operator, effectiveValue, ds.targetValue);
                                                                                    }) ?? false;

                                                                                    return (
                                                                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                                                                                            <span className="text-[10px] md:text-[11px] font-bold text-white/40 uppercase tracking-widest">Check Summary</span>
                                                                                            <div className="flex items-center gap-3">
                                                                                                <span className="text-[9px] md:text-[10px] text-white/40 uppercase font-black tracking-tighter">Result:</span>
                                                                                                <span className={`px-4 py-1.5 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest border shadow-lg ${isAuditPassed ? 'bg-violet-500/20 text-violet-100 border-violet-400/30' : 'bg-red-500/20 text-red-400 border-red-500/30'}`}>
                                                                        ...
                                                                        <div className="p-10 text-center border-2 border-dashed border-white/5 rounded-2xl bg-white/[0.01]">
                                                                            ...
                                                                            <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">Check the outcome to unlock these details</p>
                                                                        </div>
                                                                    )}

                                                                    {/* Resolution math */}
                                                                    <div>
                                                                        <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.25em] mb-4">Resolution Math</div>
                                                                        <code className="text-[13px] font-mono text-violet-100 font-bold bg-violet-500/[0.03] px-4 py-3 rounded-xl block border border-violet-400/10 shadow-inner">
                                                                            {generateMathLogicDisplay(proposal.usedDataSources, proposal.mathematicalLogic)}
                                                                        </code>
                                                                        <div className="mt-6 p-4 rounded-xl bg-violet-500/[0.03] border border-violet-400/10 flex gap-4 items-start">
                                                                            <div className="p-2 bg-violet-500/10 rounded-lg">
                                                                                <Clock className="w-4 h-4 text-violet-100" />
                                                                            </div>
                                                                            <div>
                                                                                <p className="text-[11px] font-black text-violet-100 uppercase tracking-widest mb-1">Settlement Rule</p>
                                                                                <p className="text-[13px] text-white/60 font-medium leading-relaxed">
                                                                                    This market settles only when the recorded oracle conditions are <span className="text-white font-bold italic text-violet-100">satisfied exactly at the resolution deadline</span>.
                                                                                </p>
                                                                            </div>
                                                                        </div>
                                                                    </div>
