...
export async function GET(_request: NextRequest, context: { params: Promise<{ walletAddress: string }> }) {
  ...
  try {
    const response = await fetch(backendUrl(`/reputation-records/${encodeURIComponent(walletAddress)}`), {
      method: "GET",
      cache: "no-store",
    });

    const payload = await response.json().catch(() => null);
    return NextResponse.json(payload ?? { records: [] }, { status: response.status });
  } catch {
