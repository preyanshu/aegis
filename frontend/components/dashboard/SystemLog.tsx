...
export function SystemLog({ agents }: SystemLogProps) {
            ...
            if (part.match(urlRegex)) {
                ...
                return (
                    <a
                        ...
                        className="mt-2 inline-flex w-fit items-center gap-2 rounded-lg border border-violet-400/10 bg-violet-500/5 px-3 py-1.5 text-violet-200 transition-all hover:bg-violet-500/10 group/link"
                    >
    ...
    const getLevelConfig = (level: string) => {
        switch (level.toLowerCase()) {
            ...
            case 'debug': return { icon: Bug, color: 'text-violet-300/55', bg: 'bg-violet-500/5', border: 'border-violet-400/10', label: 'VERBOSE' };
            case 'info': return { icon: Info, color: 'text-violet-200', bg: 'bg-violet-500/10', border: 'border-violet-400/20', label: 'SECURE' };
        ...
        <div className="flex flex-col h-full bg-transparent font-sans">
            ...
            <div className="p-6 pb-2 bg-transparent">
                ...
                <div className="relative group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-white/20 transition-colors group-focus-within:text-violet-300" />
                    <input
                        type="text"
                        value={filterQuery}
                        onChange={(e) => setFilterQuery(e.target.value)}
                        placeholder="Trace protocol operations..."
                        className="w-full rounded-xl border border-white/5 bg-[#121214] py-3 pl-11 pr-4 text-[11px] font-medium uppercase tracking-wider text-white/80 transition-all placeholder:text-white/10 focus:bg-white/[0.02] focus:outline-none focus:border-violet-400/20"
                    />
                </div>
            </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="bg-[#121214]/30 backdrop-blur-xl border border-white/5 rounded-2xl p-4 hover:bg-white/[0.04] transition-all duration-500">
                                            ...
                                            <div className="flex flex-col break-words font-mono text-[12px] font-medium leading-relaxed text-white/60 selection:bg-violet-500/30">
            ...
            <div className="p-4 bg-[#050507]/40 backdrop-blur-xl border-t border-white/5 flex items-center justify-between mx-2 mb-2 rounded-2xl">
                <div className="flex items-center gap-3">
                    <div className="flex -space-x-1">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-1.5 w-1.5 rounded-full border border-violet-400/30 bg-violet-500/20" />
                        ))}
                    </div>
