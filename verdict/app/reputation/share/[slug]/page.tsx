...
import { BarChart3, CircleDollarSign, Copy, Loader2, ShieldCheck, TrendingUp, Users, X } from "lucide-react";
...
function credentialIcon(metric?: string) {
  if (metric === "winRate") return <TrendingUp className="h-4 w-4" />;
  if (metric === "roi") return <BarChart3 className="h-4 w-4" />;
  if (metric === "profit") return <CircleDollarSign className="h-4 w-4" />;
  if (metric === "participation") return <Users className="h-4 w-4" />;
  return <ShieldCheck className="h-4 w-4" />;
}

function credentialAccent(metric?: string) {
...
function PublicShowcaseCardView({
  ...
  return (
    <div className="relative flex h-full flex-col rounded-[28px] border border-white/6 bg-[#121214]/80 p-5 shadow-[0_18px_50px_rgba(0,0,0,0.28)]">
      ...
      <div className="mt-5 flex gap-3">
        <div className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border ${credentialAccent(card.claim.claimType === "threshold" ? card.claim.metric : undefined)}`}>
          {credentialIcon(card.claim.claimType === "threshold" ? card.claim.metric : undefined)}
        </div>
...
export default function ReputationSharePage({ params }: { params: { slug: string } }) {
  ...
  const profileBio = share?.snapshot.profile.bio || "Trading on Verdict with a public reputation profile.";
  ...
  const totalCategories = summary?.totalCategories ?? new Set(legacyPositions.map((position) => position.category.toLowerCase())).size;
  const categoryTags = summary?.categories?.length
    ? summary.categories
    : [...new Set(legacyPositions.map((position) => position.category.toLowerCase()))].slice(0, 4);
              ...
              <aside className="px-2 pt-2">
                <div className="flex items-start gap-6">
                  <div className="flex h-24 w-24 shrink-0 aspect-square items-center justify-center overflow-hidden rounded-full border border-white/5 bg-[#121214] text-2xl font-black uppercase text-white">
                    {share?.snapshot.profile.avatarDataUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={share.snapshot.profile.avatarDataUrl} alt={profileName} className="block h-full w-full min-h-full min-w-full rounded-full object-cover" />
                    ) : (
                      profileName.slice(0, 1)
                    )}
                  </div>
