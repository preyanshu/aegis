...
import { BarChart3, ChevronDown, CircleDollarSign, Copy, HelpCircle, Loader2, MoreVertical, Share2, ShieldCheck, Sparkles, TrendingUp, Users, X } from "lucide-react";
...
function credentialIconForCard(card: ShowcaseCredentialCard) {
  const metric = card.claim?.metric;
  if (metric === "winRate") return <TrendingUp className="h-4.5 w-4.5" />;
  if (metric === "roi") return <BarChart3 className="h-4.5 w-4.5" />;
  if (metric === "profit") return <CircleDollarSign className="h-4.5 w-4.5" />;
  if (metric === "participation") return <Users className="h-4.5 w-4.5" />;
  return <ShieldCheck className="h-4.5 w-4.5" />;
}

function credentialAccentForCard(card: ShowcaseCredentialCard) {
  const metric = card.claim?.metric;
  if (metric === "winRate") {
...
function SortableCredentialCard({
  ...
  return (
    ...
    >
      <div
        className={`relative flex h-full flex-col rounded-[28px] border border-white/6 bg-[#121214]/80 p-5 shadow-[0_18px_50px_rgba(0,0,0,0.28)] transition ${
          isDragging ? "cursor-grabbing opacity-90 shadow-[0_28px_60px_rgba(0,0,0,0.42)]" : "cursor-grab"
        }`}
      >
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex items-center gap-2">
              <button
                ...
                className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/55 transition hover:bg-white/[0.08] hover:text-white"
        ...
        <div className="mt-5 flex gap-3">
          <div className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border ${credentialAccentForCard(card)}`}>
            {credentialIconForCard(card)}
          </div>

          <div className="min-w-0 flex-1">
            <h3 className="text-lg font-black leading-tight text-white sm:text-xl">{card.title}</h3>
            <p className="mt-1.5 text-xs leading-relaxed text-white/68">{card.description}</p>
            <p className="mt-2.5 text-xs text-white/40">{card.meta}</p>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {card.archivedAt ? (
            <span className="rounded-xl border border-amber-400/20 bg-amber-400/10 px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] text-amber-200">
          ...
          <div className="pt-4">
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <button
                ...
                className="inline-flex h-9 items-center justify-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-500/10 px-3 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-200 transition hover:bg-emerald-500/15 disabled:cursor-not-allowed disabled:opacity-60"
              >
            ...
            <div className="px-5 py-5">
              <div className="rounded-2xl border border-white/8 bg-white/[0.04] px-4 py-3">
                ...
                <p className="mt-2 text-sm font-semibold text-white">
                  {card.publicClaim?.statement ?? card.title}
                </p>
              </div>
...
export default function ReputationPage() {
  ...
  const pendingAttestationPositions = useMemo(
    () => walletPositions.filter((position) => position.claimedAt && position.reputationAttestationStatus !== "attested"),
    [walletPositions],
  );
  ...
  const generationStep = generationStepIndex >= 0 ? generationPhases[generationStepIndex] : "Generating credential";
